...
def invoice_creates(request):
    ...
    if request.method == 'POST':
        ...
        if validation_error:
            context["error_message"] = validation_error
            return render(request, 'gstbillingapp/invoice_creates.html', context)
        ...
        return redirect('invoice_viewers', invoice_id=new_invoice.id)

    return render(request, 'gstbillingapp/invoice_creates.html', context)


@login_required
def invoicess(request):
    context = {}
    context['invoicess'] = Invoice.objects.filter(user=request.user).order_by('-id')
    return render(request, 'gstbillingapp/invoicess.html', context)


@login_required
def invoice_viewers(request, invoice_id):
    invoice_obj = get_object_or_404(Invoice, user=request.user, id=invoice_id)
    user_profile = get_object_or_404(UserProfile, user=request.user)
    ...
    context['total_in_words'] = num2words.num2words(int(context['invoice_data']['invoice_total_amt_with_gst']), lang='en_IN').title()
    context['user_profile'] = user_profile
    return render(request, 'gstbillingapp/invoice_printers.html', context)


@login_required
def invoice_deletes(request):
    if request.method == "POST":
        invoice_id = request.POST["invoice_id"]
        invoice_obj = get_object_or_404(Invoice, user=request.user, id=invoice_id)
        if len(request.POST.getlist('inventory-del')):
